...
import "github.com/Daviddlh1/writing-an-interpreter-in-go/pkg/token"

/*
The Lexer struct is responsible for tokenizing the input string.
It keeps track of the current position, the next reading position,
and the current character being examined.
*/
type Lexer struct {
	input        string
	position     int  // current position in input (points to current char)
	readPosition int  // current reading position in input (after current char or the next char in our input)
